   ...
   lateinit var longitude:String
    ...
    fun fetchData() :LiveData<WeatherResponse>{
        ...
        CoroutineScope(Dispatchers.IO + exceptionHandlerException).launch {
            val response = NetworkService.getCurrentData().getCurrentWeatherData(latitude,longitude,"minutely",units,lang)
    ...
    fun setData(lat:String,lng:String){
        ...
        CoroutineScope(Dispatchers.IO + exceptionHandlerException).launch {
            val response = NetworkService.getCurrentData().getCurrentWeatherData(lat,lng,"minutely",units,lang)
            withContext(Dispatchers.Main) {
                if (response.isSuccessful) {
                    localDataSource.insert(response.body())
                    //  weatherLiveData.postValue(response.body())
